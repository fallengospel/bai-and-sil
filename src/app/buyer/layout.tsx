...
export default function BuyerLayout({ children }: { children: React.ReactNode }) {
  ...
  if (loading) return <div className="flex items-center justify-center h-screen"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#7BA8D0]" /></div>;
  ...
  return (
    <div className="flex min-h-screen">
      <aside className="w-64 border-r bg-gray-50 hidden md:block">
        <div className="p-6">
          <h2 className="text-lg font-bold text-[#7BA8D0]">Buyer Hub</h2>
          ...
          {sidebarLinks.map((link) => (
            <Link key={link.href} href={link.href}
              className={cn('flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-colors',
                link.href === '/buyer/dashboard' ? 'bg-[#7BA8D0]/10 text-[#7BA8D0]' : 'text-gray-600 hover:bg-gray-100'
              )}>
